...
public class SpringmvcServiceBootstrap implements ApplicationRunner {
    ...
    private void makeDispatcherController(MybatisApplicationOptions applicationOptions) {
        String application = applicationOptions.getId();
//        Class<?> dispatcherController = doMakeDispatcherController(applicationOptions);

        Map<String, List<MappedStatement>> collect = applicationOptions.getMappedStatements().stream().collect(Collectors.groupingBy(ms ->
                ms.getId().lastIndexOf(".") != -1 ?
                        ms.getId().substring(0, ms.getId().lastIndexOf(".")) :
                        ms.getId()));
        System.out.println(collect);

        //需要将方法进行分类，相同的namespace的方法放到一起后，一起生成一个类，然后在执行路径上添加应用名。

//        ac.registerBean("springmvc.MapperController", dispatcherController);
...
//    private Class<?> doMakeDispatcherController(MybatisApplicationOptions applicationOptions) {
